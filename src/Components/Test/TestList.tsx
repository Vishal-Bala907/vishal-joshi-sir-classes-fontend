...
const TestList = ({ type, reload }: { type: string, reload: number }) => {
    ...
    const [selectedTestLeaderBoardId, setSelectedLeaderBoardId] = useState<any>(false)
    ...
    return (
        <Row>
                                ...
                                tests?.map((test: any, index: number) => (
                                    <Fragment key={index}>
                                        <li className="d-flex align-items-center gap-2" key={index}>
                                            ...
                                            <div className={`flex-grow-1 border-2 b-l-success p-1`} style={{ cursor: 'pointer' }}>
                                                <p onClick={() => router.push(`/${test._id}`)}>{test.name}</p>
                                            </div>
                                            {
                                                selectedTestLeaderBoardId == test._id ?
                                                    <div onClick={() => { setSelectedLeaderBoardId(false) }}>
                                                        <Icon icon="ic:round-close" width="24" height="24" />
                                                    </div>
                                                    :
                                                    <div onClick={() => { setSelectedLeaderBoardId(test._id) }}>
                                                        <Icon icon="material-symbols:leaderboard-rounded" width="24" height="24" />
                                                    </div>
                                            }
                                            {
                                                downloading == index ?
                                                    ...
                                                    <div className={`p-1`} style={{ cursor: 'pointer' }} onClick={async () => {
                                                        setDownloading(index)
                                                        handleTestQuestionDownload(test._id)
                                                            .finally(() => { setDownloading(Math.random()) })
                                                    }
                                                    }>
                                                        <Icon icon="material-symbols:download-2-rounded" width="24" height="24" />
                                                    </div>
                                            }
                                            {
                                                user.role == "admin" &&
                                                <div onClick={() => handleTestDelete(test._id)} style={{ cursor: 'pointer' }}>
            ...
            {selectedTestLeaderBoardId && (
                <Col xxl="4" xl="4" lg="4" md="12" sm="12" className="order-xxl-0 order-xl-2 box-col-6">
                    <TestLeaderTable selectedTestLeaderBoardId={selectedTestLeaderBoardId} />
                </Col>
            )}
        </Row>
    )
